...
export default function SuperAdminBalanceFiles() {
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter((t) => String(t.id).includes(q) || t.bingoCenterUsername.toLowerCase().includes(q) || t.debitedBy.toLowerCase().includes(q))
  }, [transactions, search])
                ...
                <div className="animate-fade-up">
                  ...
                  {packages.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-slate-400">Create one in the Packages section</p>
                    </div>
                  ) : (
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-coral-100">
              ...
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Generated By</th>
      ...
      <div className="lg:hidden space-y-3">
        {filtered.map((tx) => (
          <div key={tx.id} className="bg-white rounded-2xl p-4 shadow-soft border border-coral-100/50">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-bold text-slate-900">BAL-{String(tx.id).padStart(4, '0')}</span>

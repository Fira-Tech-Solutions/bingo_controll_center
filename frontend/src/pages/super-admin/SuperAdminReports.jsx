...
export default function SuperAdminReports() {
  ...
  async function loadData() {
    ...
    setError('')
    try {
      const [t, c, p, o] = await Promise.allSettled([
        api.getTransactions(token),
        api.getCenters(token),
        api.getPackages(token, 1, 100),
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 shadow-soft border border-coral-100/50">
          <div className="text-xs uppercase tracking-wide text-slate-500 font-semibold">Total Payment</div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-soft border border-coral-100/50">
          ...
          <div className="mt-2 text-2xl font-extrabold text-slate-900">{filtered.length}</div>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-soft border border-coral-100/50">
          <div className="text-xs uppercase tracking-wide text-slate-500 font-semibold">Avg Payment</div>
          <div className="mt-2 text-2xl font-extrabold text-slate-900">{formatAmount(avgPayment)}</div>
        </div>
      </section>
        ...
        {byCenter.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-coral-100"><th className="text-left py-2 font-bold text-slate-500">Center</th><th className="text-right py-2 font-bold text-slate-500">Txns</th><th className="text-right py-2 font-bold text-slate-500">Paid</th><th className="text-right py-2 font-bold text-slate-500">Issued</th></tr></thead>
              <tbody>{byCenter.map(([name, d]) => <tr key={name} className="border-b border-coral-100/50"><td className="py-2 font-semibold">{name}</td><td className="text-right">{d.count}</td><td className="text-right font-bold">{formatAmount(d.paid)}</td><td className="text-right font-bold text-coral-600">{formatAmount(d.issued)}</td></tr>)}</tbody>
            </table>
          </div>
        ...
        {byAgent.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>{byAgent.map(([name, d]) => <tr key={name} className="border-b border-coral-100/50"><td className="py-2 font-semibold">{name}</td><td className="text-right">{d.count}</td><td className="text-right font-bold">{formatAmount(d.paid)}</td><td className="text-right font-bold text-coral-600">{formatAmount(d.issued)}</td></tr>)}</tbody>
            </table>
          </div>
        ) : <EmptyState text="No data for this period." />}
      </section>

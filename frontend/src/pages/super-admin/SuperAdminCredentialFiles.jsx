...
export default function SuperAdminCredentialFiles() {
  const { token } = useAuth()
  ...
  const filtered = useMemo(() => {
    ...
    return centers.filter((c) => c.full_name.toLowerCase().includes(q) || c.username.toLowerCase().includes(q) || c.mac_address.toLowerCase().includes(q))
  }, [centers, search])
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-coral-100">
              ...
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-500">MAC Address</th>
              <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Created By</th>
              <th className="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-slate-500">Created</th>
              <th className="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-slate-500">Status</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((c) => (
              <tr key={c.username} className="border-b border-coral-100/50 hover:bg-coral-50/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-slate-500 font-mono">{c.username}</div>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <span className="text-xs font-bold px-2.5 py-1 rounded-full border bg-emerald-100 text-emerald-700 border-emerald-200">Active</span>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {filtered.map((c) => (
          <div key={c.username} className="bg-white rounded-2xl p-4 shadow-soft border border-coral-100/50">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-bold text-slate-900">CRED-{c.username}</span>
              <span className="text-xs font-bold px-2.5 py-1 rounded-full border bg-emerald-100 text-emerald-700 border-emerald-200">Active</span>
            </div>
            ...
            <div className="text-xs text-slate-500 mt-1">Created by {c.createdBy} • {formatDate(c.createdAt)}</div>
          </div>
        ))}
        {filtered.length === 0 && <EmptyState text="No credential files found." />}
      </div>

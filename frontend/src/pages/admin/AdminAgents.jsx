...
export default function AdminAgents() {
  ...
  const agentsWithStats = useMemo(() => {
    return operators.map((op) => {
      ...
      return { ...op, agentTxns, totalIssued, lastTxn, txnCount: agentTxns.length }
    })
  ...
  function handleFormChange(e) { setForm((s) => ({ ...s, [e.target.name]: e.target.value })) }
        ...
        <table className="w-full">
          ...
          <tbody>{filtered.map((a) => (
            <tr key={a.username} className="border-b border-coral-100/50 hover:bg-coral-50/30 transition-colors">
              <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-coral-100 text-coral-600 flex items-center justify-center font-bold text-sm">{a.full_name?.charAt(0)}</div><div><div className="font-bold text-sm text-slate-900">{a.full_name}</div><div className="text-xs text-slate-500 font-mono">{a.username}</div></div></div></td>
              <td className="px-6 py-4"><span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${a.isBanned ? 'bg-red-100 text-red-700 border-red-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}>{a.isBanned ? 'Banned' : 'Active'}</span></td>
          ...
          <div key={a.username} className="bg-white rounded-2xl p-4 shadow-soft border border-coral-100/50">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0"><div className="font-bold text-slate-900 truncate">{a.full_name}</div><div className="text-xs text-slate-500 font-mono truncate">{a.username}</div></div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 text-center">
              <div className="p-2 rounded-lg bg-coral-50/50"><div className="text-xs text-slate-500">Transactions</div><div className="font-bold text-sm text-slate-900">{a.txnCount}</div></div>
              <div className="p-2 rounded-lg bg-coral-50/50"><div className="text-xs text-slate-500">Issued</div><div className="font-bold text-sm text-slate-900">{formatAmount(a.totalIssued)}</div></div>
            </div>

...
export default function AdminCenters() {
  ...
  const centersWithStats = useMemo(() => {
    return centers.map((c) => {
      ...
      return { ...c, lastTxn, totalPaid, status, txnCount: ct.length }
    })
  ...
  async function handleCreate() {
    ...
    setError('')
    try {
      const res = await api.createCenter(token, {
        ...
        additional_contact: form.additional_contact || null,
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-lg bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl max-h-[92vh] flex flex-col animate-slide-up overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input name="full_name" value={form.full_name} onChange={handleFormChange}
                        className="w-full h-12 px-4 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 transition-all"
                        placeholder="e.g. Merkato Branch" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">Username</label>
                        <input name="username" value={form.username} onChange={handleFormChange}
                          className="w-full h-12 px-4 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 font-mono transition-all"
                          placeholder="merkato01" />
                      </div>
                      <div>
                    ...
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">MAC Address</label>
                      <input name="mac_address" value={form.mac_address} onChange={handleFormChange}
                        className="w-full h-12 px-4 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 font-mono transition-all"
              ...
              {/* Step 1: Contact / Registration Info */}
              {step === 1 && (
                <div className="animate-fade-up space-y-5">
                  <div className="text-center">
                    <div className="w-14 h-14 mx-auto rounded-2xl bg-coral-50 flex items-center justify-center mb-3">
                      <span className="material-symbols-outlined text-[28px] text-coral-500">contact_mail</span>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">Owner Name</label>
                      <input name="owner_name" value={form.owner_name} onChange={handleFormChange}
                        className="w-full h-12 px-4 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 transition-all"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">Additional Contact</label>
                        <input name="additional_contact" value={form.additional_contact} onChange={handleFormChange}
                          className="w-full h-12 px-4 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 transition-all"
                          placeholder="Alt phone, telegram, etc." />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">Region / City</label>
                        <input name="region" value={form.region} onChange={handleFormChange}
                          className="w-full h-12 px-4 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 transition-all"
                          placeholder="e.g. Addis Ababa" />
                      </div>
                      <div>
                        <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">Address</label>
                    ...
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">Notes</label>
                      <textarea name="notes" value={form.notes} onChange={handleFormChange} rows={3}
                        className="w-full px-4 py-3 rounded-2xl bg-slate-50 focus:bg-white outline-none border-2 border-slate-100 focus:border-coral-400 text-sm font-medium text-slate-800 transition-all resize-none"
                        placeholder="Any additional registration notes..." />
                    </div>
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="animate-fade-up space-y-5">
                  <div className="text-center">
                    <div className="w-14 h-14 mx-auto rounded-2xl bg-coral-50 flex items-center justify-center mb-3">
                      <span className="material-symbols-outlined text-[28px] text-coral-500">fact_check</span>
                    </div>
                    <h3 className="font-extrabold text-xl text-slate-900">Review & Confirm</h3>
                    <p className="text-sm text-slate-500 mt-1">Confirm the center details before creating</p>
                  </div>
              ...
              {step === 3 && createResult && (
                <div className="animate-fade-up text-center space-y-4">
                  <div className="w-20 h-20 mx-auto rounded-full bg-emerald-50 flex items-center justify-center animate-pop-in">
                    <svg className="w-10 h-10 success-check" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><polyline points="8 12 11 15 16 9" /></svg>
                  </div>
                  <div>
                    <h3 className="font-extrabold text-xl text-slate-900">Center Created!</h3>
                    <p className="text-sm text-slate-500 mt-1">Encrypted terminal file has been generated</p>
                  </div>
                  ...
                  {createResult.encryptedFile && (
                    <button onClick={() => downloadEncryptedFile(createResult.encryptedFile)}
                      className="w-full py-3 rounded-2xl bg-coral-500 text-white font-bold text-sm shadow-lg shadow-coral-500/25 hover:bg-coral-600 active:scale-[0.97] transition-all flex items-center justify-center gap-2">
            ...
            {step < 3 && (
              <div className="shrink-0 px-5 py-4 border-t border-slate-100 flex gap-3 bg-white">
                {step > 0 && (
                  <button onClick={goBack} className="px-5 py-3 rounded-2xl bg-slate-100 text-slate-600 font-semibold text-sm hover:bg-slate-200 active:scale-[0.97] transition-all">Back</button>
                )}
                <button onClick={step === 2 ? handleCreate : nextStep}
                  disabled={creating}
                  ...
                  {creating ? (
                    ...
                    <><span className="material-symbols-outlined text-[18px]">check_circle</span>Create Center</>
                  ) : (
            ...
            {step === 3 && (
              <div className="shrink-0 px-5 py-4 border-t border-slate-100 bg-white">
                <button onClick={closeWizard} className="w-full py-3 rounded-2xl bg-slate-900 text-white font-bold text-sm hover:bg-slate-800 active:scale-[0.97] transition-all">Done</button>
              </div>
            )}
          </div>
        </div>

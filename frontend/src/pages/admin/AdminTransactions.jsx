  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-white rounded-2xl p-6 shadow-soft border border-coral-100/50">
        ...
        <p className="text-sm text-slate-500 mt-1">{filtered.length} transaction(s) • Your scoped ledger</p>
        <div className="flex gap-3 mt-3 text-sm">
          <div className="px-4 py-2 rounded-xl bg-coral-50 border border-coral-100"><span className="text-xs text-slate-500">Paid</span><span className="ml-2 font-bold text-slate-900">{formatAmount(totalPaid)}</span></div>
          <div className="px-4 py-2 rounded-xl bg-coral-50 border border-coral-100"><span className="text-xs text-slate-500">Issued</span><span className="ml-2 font-bold text-coral-600">{formatAmount(totalIssued)}</span></div>
        </div>
      </section>

      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-[20px]">search</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by ID, center, user..." className="w-full h-11 pl-10 pr-4 rounded-xl bg-white border border-coral-100 text-sm font-medium focus:outline-none focus:border-coral-500" /></div>
        <select value={centerFilter} onChange={(e) => setCenterFilter(e.target.value)} className="h-11 px-4 rounded-xl bg-white border border-coral-100 text-sm font-medium focus:outline-none focus:border-coral-500"><option value="">All Centers</option>{centers.map((c) => <option key={c.username} value={c.username}>{c.full_name}</option>)}</select>
      ...
      <div className="lg:hidden space-y-3">
        {filtered.map((tx) => (
          <div key={tx.id} className="bg-white rounded-2xl p-4 shadow-soft border border-coral-100/50">
            <div className="flex items-center justify-between"><span className="text-xs font-mono font-bold text-slate-900">TX-{String(tx.id).padStart(4, '0')}</span><span className="text-xs text-slate-500">{formatDate(tx.timestamp)}</span></div>
            <div className="font-semibold text-sm text-slate-900 mt-1">{tx.bingoCenterUsername}</div>
            <div className="grid grid-cols-2 gap-3 mt-2">
              <div className="p-2 rounded-lg bg-coral-50/50 text-center"><div className="text-xs text-slate-500">Paid</div><div className="font-bold text-sm text-slate-900">{formatAmount(tx.actualAmount)}</div></div>
              <div className="p-2 rounded-lg bg-coral-50/50 text-center"><div className="text-xs text-slate-500">Issued</div><div className="font-bold text-sm text-coral-600">{formatAmount(tx.generatedAmount)}</div></div>
            </div>
